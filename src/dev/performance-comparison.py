...
data['Peak memory usage [MB]'] = data['Peak memory usage']
data = data.sort_values(by=['Execution time (s) / row'])

fig, ax = plt.subplots(nrows=2, ncols=2)
plt.suptitle('Performance Comparison', fontsize=24)
...
sns.barplot(data, x='Peak memory usage [MB]', y='File mode', ax=ax[1][0])
sns.barplot(data, x='Peak memory usage [MB]', y='Processing mode', ax=ax[1][1])
plt.show()
...
plt.title('Table Completeness by Column', fontsize=24)
plt.show()
...
plt.yscale('log')
plt.title('Row Count by Year', fontsize=24)
plt.show()